...
export const content: Record<Lang, SiteContent> = {
  ja: {
    ...
    hero: {
      ...
      requirement: 'Windows 10 / 11 と macOS のアプリです。スマートフォンではご利用いただけません。上のボタンは Windows 版で、Mac 版はいま配布の準備をしています。',
    ...
    tokushoho: {
      ...
      rows: [
        ...
        { label: '動作環境', value: 'Windows 10 / 11（64bit）および macOS（Intel / Apple Silicon）。Mac 版は現在準備中です。Windows 11 の「スマート アプリ コントロール」が有効な環境では、署名のないアプリとして起動が拒否されます（警告は表示されません）。該当する場合は Microsoft ストア版の公開をお待ちください。' },
      ],
    },
  ...
  en: {
    ...
    hero: {
      ...
      requirement: 'For Windows 10 / 11 and macOS. This app does not run on smartphones. The button above is the Windows version; the Mac version is being prepared for release.',
