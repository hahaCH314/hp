...
**ただし、運営者や作者の個人的な属性はこのリポジトリのどこにも書かない**（コメントもドキュメントも）。リポジトリは公開先に残り、一度書くと履歴からも消しにくい。特商法ページに何を出し何を伏せるかは上のルールだけで判断でき、背景の事情は要らない。

## 販売

**2026-08-24 に無料・直接ダウンロードへ切り替えた。** 前は ja が BOOTH（¥500）、en が Ko-fi（$5）だったが、伊波さん「BOOTHって馴染みがなさすぎて」「直接ダウンロードが１番いい」の判断で、有料販売そのものをやめた。

ダウンロードボタンは押したらそのままファイルが落ちる直リンク（GitHub Releases、`CMCUBE-Setup.exe`）。URL は `App` に直書きせず、文言と同じく `hero.buyHref` として言語ごとに `content.ts` が持つ。**URL にバージョンを入れないこと**（`latest` + 固定ファイル名で受けている。新しい版を出してもここは直さなくてよい。上げるときはファイル名をそろえること）。

BOOTH / Ko-fi は今は使っていない。**有料の要素を復活させたときは、`hero.buyLabel`/`buyHref` と `tokushoho` の価格欄（販売価格・支払方法・支払時期・引渡時期・返品）を必ず一緒に直すこと**（`content.ts` 該当行にも同じ注記あり）。
